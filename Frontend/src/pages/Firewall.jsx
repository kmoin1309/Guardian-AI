    ...
    <div className="flex min-h-screen bg-[#020617] text-slate-300 font-sans selection:bg-blue-500/30 overflow-hidden">
      ...
      <aside className="fixed left-0 top-0 h-screen w-64 bg-[#0A0A14]/80 backdrop-blur-xl border-r border-slate-800/50 p-6 flex flex-col z-20 transition-all duration-300">
        ...
        <nav className="space-y-2 flex-1">
          ...
          <NavButton icon={AlertTriangle} label="Jailbreak Detector" onClick={() => navigate('/jailbreak-detector')} />
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {scanResult.threats_detected.map((threat, idx) => {
                              const isObject = typeof threat === 'object';
                              const threatType = isObject ? (threat.type || threat.pattern || 'Unknown Threat') : threat;
                              const threatRisk = isObject ? threat.risk : null;
                              const threatMatches = isObject ? threat.matches : null;
                                ...
                                <div key={idx} className="bg-red-500/10 border border-red-500/20 rounded-lg p-3">
                                  <div className="flex items-center justify-between mb-1">
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span className="text-sm font-bold text-red-300">{threatType}</span>
                                    </div>
                                    {threatRisk && (
                                      <span className="text-[10px] font-black px-2 py-0.5 bg-red-600/20 text-red-400 rounded uppercase">
                                        {threatRisk}
                                      </span>
                                    )}
                                  </div>
                                  {isObject && threat.pattern && (
                                    <div className="text-[10px] text-slate-400 mt-1 font-mono">
                                      Pattern: <code className="text-orange-300">{threat.pattern}</code>
                                    </div>
                                  )}
                                  {threatMatches > 0 && (
                    ...
                    <div key={scan.id || index} className="bg-slate-900/50 border border-slate-700/50 hover:border-slate-600 p-3 rounded-xl transition-colors group cursor-default">
                      ...
                      <div className="flex items-center justify-between mt-1 pt-2 border-t border-slate-800/50">
                        <span className="text-[10px] text-slate-500">
                          {(() => {
                            if (typeof scan.threats === 'number') {
                              return scan.threats > 0 ? `${scan.threats} detected` : 'Clean';
                            } else if (Array.isArray(scan.threats)) {
                              return scan.threats.length > 0 ? `${scan.threats.length} detected` : 'Clean';
                            } else if (typeof scan.threats === 'object' && scan.threats !== null) {
                              return scan.threats.count > 0 ? `${scan.threats.count} detected` : 'Clean';
                            }
                            return 'Clean';
                          })()}
                        </span>
