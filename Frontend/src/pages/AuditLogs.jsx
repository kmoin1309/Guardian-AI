...
const AuditLogs = () => {
  ...
  const getStatusBadge = (status) => {
    const badges = {
      ...
      'IN_REVIEW': { color: 'bg-yellow-500/20 text-yellow-400', text: 'IN REVIEW' },
    ...
    <div className="min-h-screen bg-[#0a0f1e]">
      ...
      <aside className="fixed left-0 top-0 h-screen w-64 bg-[#0B1120] border-r border-gray-800 p-6">
        ...
        <nav className="space-y-2">
          ...
          <a href="/audit-logs" className="flex items-center gap-3 px-4 py-3 bg-blue-600/10 text-blue-400 rounded-lg font-medium">
            <span>📜</span>
      ...
      <main className="ml-64 p-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-3xl font-black text-white mb-2">Audit Logging & History</h1>
              <p className="text-gray-400">Comprehensive real-time activity for OWASP monitoring and compliance</p>
            </div>
        ...
        <div className="bg-[#0f1629] rounded-xl border border-gray-800 p-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            <button 
              onClick={() => setFilters({ severity: 'all', status: 'all', eventType: 'all' })}
          ...
          <div className="col-span-2 bg-[#0f1629] rounded-xl border border-gray-800 p-6">
              ...
              {currentLogs.map((log) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`px-3 py-1 rounded text-xs font-bold ${getSeverityColor(log.eventType)}`}>
                      {getEventIcon(log.eventType)} {log.eventType.replace('_', ' ')}
                    </span>
                  </div>
                  ...
                  <div>
                    <span className={`px-3 py-1 rounded text-xs font-bold ${
                      log.impact === 'CRITICAL' ? 'bg-red-500/20 text-red-400' :
                      log.impact === 'HIGH' ? 'bg-orange-500/20 text-orange-400' :
                      log.impact === 'MEDIUM' ? 'bg-yellow-500/20 text-yellow-400' :
                  ...
                  <div>
                    <span className={`px-3 py-1 rounded text-xs font-bold ${getStatusBadge(log.status).color}`}>
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-800">
              <div className="text-gray-400 text-sm">
                Showing {indexOfFirstLog + 1} to {Math.min(indexOfLastLog, filteredLogs.length)} of {filteredLogs.length} events
              </div>
              <div className="flex items-center gap-2">
                <button 
            ...
            {selectedLog ? (
              <>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="text-xs font-bold text-blue-400 mb-1">EVENT ID</div>
                    <div className="text-white font-mono text-sm">evt_id_{selectedLog.id}_at8bd2</div>
                  </div>

                  <div>
                    <div className="text-xs font-bold text-blue-400 mb-1">CATEGORY</div>
                    <div className="text-white text-sm">{selectedLog.category}</div>
                  </div>

                  <div>
                    <div className="text-xs font-bold text-blue-400 mb-1">DESCRIPTION</div>
                    <div className="text-gray-300 text-sm">{selectedLog.description}</div>
                  </div>

                  <div className="pt-4 border-t border-gray-800">
                    <div className="text-xs font-bold text-blue-400 mb-3">RAW JSON METADATA</div>
                  ...
                  <div className="pt-4 border-t border-gray-800">
                    <div className="text-xs font-bold text-blue-400 mb-3">SECURITY TRACE</div>
                    <div className="space-y-2">
              ...
              <div className="text-center py-12 text-gray-500">
                ...
                <div className="text-sm">Click on a log entry to view details</div>
              </div>
            )}
          </div>
        </div>
