...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:8000/api/dashboard/realtime', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
  ...
  const testConnection = async (modelId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8000/api/llm/test/${modelId}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const deleteConnection = async (modelId, modelName) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:8000/api/llm/models/${modelId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
    ...
    <div className="min-h-screen bg-[#020617]">
      ...
      <aside className="fixed left-0 top-0 h-screen w-64 bg-[#0B1120] border-r border-gray-800 p-6">
        ...
        <nav className="space-y-2">
          <a href="/dashboard" className="flex items-center gap-3 px-4 py-3 bg-blue-600/10 text-blue-400 rounded-lg font-medium">
            <span>📊</span>
            <span>Command Center</span>
          </a>
          
          <a href="/protection" className="flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-white hover:bg-gray-800/50 rounded-lg font-medium">
            <span>🛡️</span>
            <span>Protection</span>
          </a>
         
          <a href="/audit-logs" className="flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-white hover:bg-gray-800/50 rounded-lg font-medium">
            <span>📜</span>
            <span>Audit Logs</span>
          </a>
         
          <a href="/guidelines" className="flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-white hover:bg-gray-800/50 rounded-lg font-medium">
            <span>🎓</span>
      ...
      <main className="ml-64 p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <div className="flex items-center gap-3">
              <p className="text-gray-400">Real-time monitoring of model ingress and egress traffic</p>
              {lastUpdate && (
        ...
        <div className="mb-8 bg-[#0B1120] rounded-2xl border border-gray-800 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-400 text-sm">Manage your AI model endpoints</p>
            </div>
            <button
          ...
          {llmConnections.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              ...
              <div className="text-sm">Connect your first LLM to start security testing</div>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 gap-4">
              {llmConnections.map((conn) => (
                <div key={conn.id} className="bg-[#020617] border border-gray-800 rounded-lg p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="font-bold text-white">{conn.model_name}</div>
                      <div className="text-gray-400 text-xs">{conn.model_type} • {conn.auth_type}</div>
                    </div>
                    <div className={`px-3 py-1 rounded-full text-xs font-bold ${
                      ...
                      {conn.health_status === 'healthy' ? '✓ Healthy' : '⚠ Unhealthy'}
                    </div>
                  </div>
                  <div className="text-gray-500 text-sm mb-3 truncate">{conn.endpoint_url}</div>
        ...
        <div className="bg-[#0B1120] rounded-2xl border border-gray-800 p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold text-white">Real-time Security Pulse</h2>
            <div className="flex gap-6">
              <div>
                <div className="text-gray-400 text-xs uppercase">Active Requests</div>
              ...
              <div>
                <div className="text-gray-400 text-xs uppercase">Blocked Threats</div>
          ...
          <div className="h-64 bg-gradient-to-b from-blue-900/20 to-transparent rounded-lg border border-gray-800 flex items-center justify-center">
            <div className="text-gray-500 text-center">
              ...
              <div className="text-sm">Connect your LLM to see real-time data</div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-[#0B1120] rounded-xl border border-gray-800 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <div className="text-xs text-gray-600">Prompt Injection Blocked</div>
              </div>
            </div>
          ...
          <div className="bg-[#0B1120] rounded-xl border border-gray-800 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <div className="text-xs text-gray-600">PII Leakage Prevented</div>
              </div>
            </div>
            <div className="text-3xl font-black text-white mb-1">
          ...
          <div className="bg-[#0B1120] rounded-xl border border-gray-800 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <div className="text-xs text-gray-600">Tool Access Violations</div>
              </div>
            </div>
            <div className="text-3xl font-black text-white mb-1">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="bg-[#0B1120] rounded-xl border border-gray-800 p-6">
            <h3 className="text-sm font-bold text-gray-400 mb-4">OWASP Risk Coverage</h3>
              ...
              <div className="relative w-32 h-32">
                <svg className="transform -rotate-90" viewBox="0 0 120 120">
                  ...
                  <circle 
                    ...
                    fill="none"
                    strokeDasharray="314" 
                    strokeDashoffset={314 - (314 * (dashboardData?.owasp_coverage || 85) / 100)} 
                    strokeLinecap="round" 
                  />
                </svg>
            ...
            <div className="space-y-2 text-sm">
              {dashboardData?.owasp_checks?.map((check, index) => (
                <div key={index} className="flex justify-between text-gray-400">
                  ...
                  <span className={check.status === 'protected' ? 'text-green-400' : 'text-yellow-400'}>
                    {check.icon}
                  </span>
                </div>
              )) || (
          ...
          <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-xl border border-blue-500 p-6">
            <h3 className="text-sm font-bold text-blue-100 mb-4">LATEST RED-TEAM SCORE</h3>
            <div className="text-6xl font-black text-white mb-2">
              {dashboardData?.red_team_score || 9.2}
              <span className="text-2xl">/{dashboardData?.red_team_max || 10}</span>
            </div>
            <div className="flex items-center gap-2 text-blue-100 text-sm mb-6">
              <span>🛡️</span>
              <span>Certified {dashboardData?.red_team_certified_days_ago || 3} days ago</span>
            </div>
          ...
          <div className="bg-[#0B1120] rounded-xl border border-gray-800 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-gray-400">Latest Security Events</h3>
              <a href="#" className="text-blue-400 text-xs font-bold hover:underline">View All →</a>
            </div>
            <div className="space-y-3">
              {dashboardData?.security_events?.map((event, index) => (
                <div key={index} className={`flex items-start gap-3 p-3 bg-${event.color}-500/10 rounded-lg border border-${event.color}-500/30`}>
                  <span className={`text-${event.color}-400`}>{event.icon}</span>
                ...
                <>
                  <div className="flex items-start gap-3 p-3 bg-red-500/10 rounded-lg border border-red-500/30">
                    ...
                    <div className="flex-1">
                      <div className="text-white text-sm font-bold">Potential jailbreak detected</div>
                      <div className="text-gray-400 text-xs">Blocked user request</div>
                    </div>
                  ...
                  <div className="flex items-start gap-3 p-3 bg-green-500/10 rounded-lg border border-green-500/30">
                    <span className="text-green-400">✓</span>
                    <div className="flex-1">
                      <div className="text-white text-sm font-bold">Sensitive data redaction applied</div>
                      <div className="text-gray-400 text-xs">Masked 3 PII instances</div>
                    </div>
                  ...
                  <div className="flex items-start gap-3 p-3 bg-blue-500/10 rounded-lg border border-blue-500/30">
                    ...
                    <div className="flex-1">
                      <div className="text-white text-sm font-bold">New RAG knowledge base indexed</div>
                      <div className="text-gray-400 text-xs">1,245 vectors added</div>
                    </div>
