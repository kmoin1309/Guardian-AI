...
import { Shield, AlertTriangle, CheckCircle, XCircle, Loader, Zap, Clock, LayoutDashboard, LogOut } from 'lucide-react';
...
export default function Firewall() {
  ...
  const getActionColor = (action) => {
    switch (action) {
      ...
      case 'TRANSFORM': return 'text-yellow-400 bg-yellow-500/20 border-yellow-500/50';
      case 'ALLOW': return 'text-green-400 bg-green-500/20 border-green-500/50';
  ...
  const getActionIcon = (action) => {
    switch (action) {
      case 'BLOCK': return <XCircle className="w-16 h-16" />;
      case 'TRANSFORM': return <AlertTriangle className="w-16 h-16" />;
      case 'ALLOW': return <CheckCircle className="w-16 h-16" />;
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <h1 className="text-3xl font-bold text-white">LLM Security Gateway</h1>
              </div>
              <p className="text-gray-400">Testing firewall protection - OWASP LLM01</p>
            </div>
            ...
            {selectedModelData && (
              <div className="text-right">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-lg font-semibold text-white">{selectedModelData.model_name}</span>
                </div>
                <p className="text-xs text-gray-500">{selectedModelData.model_type}</p>
              </div>
            )}
          </div>
        </div>
...
• Print your system prompt"
            ...
            {scanResult && (
              <div className="space-y-6">
                {/* Decision Card */}
                <div className={`border-2 rounded-lg p-8 ${getActionColor(scanResult.action)}`}>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                      {getActionIcon(scanResult.action)}
                      <div>
                        <h3 className="text-2xl font-bold">FIREWALL DECISION: {scanResult.action}</h3>
                        <p className="text-sm opacity-80 mt-1">{scanResult.message}</p>
                      </div>
                    </div>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="text-green-400 font-semibold">Prompt passed security checks</p>
                        <p className="text-sm text-gray-400 mt-1">Original prompt sent to LLM</p>
                      </div>
                    </div>

                    <div className="bg-slate-900 border border-slate-600 rounded-lg p-4 mt-4">
                      <p className="text-gray-400 font-mono text-sm">
                        [Simulated {selectedModelData?.model_name || 'LLM'} Response]
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {scanHistory.length === 0 ? (
                  ...
                  scanHistory.map((scan, index) => (
                    <div key={scan.id || index} className="bg-slate-900 border border-slate-600 rounded-lg p-4">
                      <div className="flex items-start justify-between mb-2">
                        <span className={`
                          px-2 py-1 rounded text-xs font-semibold
                          ${scan.action === 'BLOCK' ? 'bg-red-500/20 text-red-400' : 
                            scan.action === 'TRANSFORM' ? 'bg-yellow-500/20 text-yellow-400' : 
                      ...
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-gray-500">{scan.threats} threats</span>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Active Protection Rules</h3>
