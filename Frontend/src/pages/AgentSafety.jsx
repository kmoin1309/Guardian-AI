...
const AgentSafety = () => {
  ...
  const [testMode, setTestMode] = useState("Guardian AI");
  ...
  const handleTestAgent = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/agents/${selectedAgent.id}/test`,
        {
          method: "POST",
          ...
          body: JSON.stringify({
            ...
            test_type:
              testMode === "Guardian AI" ? "prompt_injection" : "sql_injection",
  ...
  const handleAgentConnected = async (newAgent) => {
    ...
    const connectedAgent = agents.find(
      (a) => a.id === newAgent.agent_id || a.id === newAgent.id,
    );
    if (connectedAgent) {
  ...
  const getOverallRiskScore = () => {
    ...
    if (selectedAgent.risk_scores && selectedAgent.risk_scores.length > 0) {
      const avg =
        selectedAgent.risk_scores.reduce((a, b) => a + b, 0) /
        selectedAgent.risk_scores.length;
                    ...
                    >
                      ...
                      <div className="text-xs text-gray-500">
                        {agent.tools?.length ||
                          ...
                          0}{" "}
                        tools • {agent.total_calls || 0} calls
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="lg:col-span-2 bg-[#0B1120] rounded-xl border border-gray-800 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setTestMode("Guardian AI")}
                  className={`px-4 py-2 rounded-lg text-sm font-bold transition ${
                    testMode === "Guardian AI"
            ...
            <div className="mb-4 p-3 bg-[#020617] rounded-lg border border-gray-700">
              {testMode === "Guardian AI" ? (
                <div className="text-sm text-gray-300">
                  ...
                  Tests if Guardian AI blocks malicious prompts BEFORE they
                  reach the agent
                </div>
              ) : (
                <div className="text-sm text-gray-300">
                  ...
                  Tests if the AGENT ITSELF is secure and would reject dangerous
                  operations
                </div>
              )}
            </div>
            ...
            {!selectedAgent ? (
              ...
              <>
                ...
                <textarea
                  ...
                  placeholder={`${
                    testMode === "Guardian AI"
                      ? "Enter malicious prompt to test Guardian AI detection..."
                ...
                {testResult && (
                  <div className="mb-4 animate-fadeIn">
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div
                          className={`text-2xl font-black ml-4 ${
                            testResult.risk_score > 70
                              ? "text-red-400"
                              : testResult.risk_score > 40
                                ? "text-yellow-400"
                                : "text-green-400"
                          }`}
                        >
                      ...
                      {/* Vulnerabilities */}
                      {testResult.details?.vulnerabilities &&
                        testResult.details.vulnerabilities.length > 0 && (
                          <div className="mb-3 p-3 bg-red-500/10 rounded border border-red-500/30">
                            ...
                            <ul className="space-y-1">
                              {testResult.details.vulnerabilities.map(
                                (vuln, i) => (
                                  <li
                                    key={i}
                      ...
                      {testResult.details?.recommendations &&
                        testResult.details.recommendations.length > 0 && (
                          <div className="p-3 bg-blue-500/10 rounded border border-blue-500/30">
                            <div className="text-sm font-bold text-blue-400 mb-2">
                              💡 Recommendations:
                            </div>
                            <ul className="space-y-1">
                              {testResult.details.recommendations.map(
                                (rec, i) => (
                                  <li
                                    key={i}
                                    className="text-sm text-gray-300 flex items-start gap-2"
                        ...
                        <div className="mt-3 flex items-center gap-2">
                          ...
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-bold ${
                              testResult.details.severity === "CRITICAL"
                                ? "bg-red-600 text-white"
                                : testResult.details.severity === "HIGH"
                                  ? "bg-orange-600 text-white"
                                  : testResult.details.severity === "MEDIUM"
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(
                      ...
                      >
                        {typeof tool === "string"
                          ? tool
                          : tool.name || tool.tool_name || "Unknown Tool"}
                      </span>
                    ))}
                    {(!selectedAgent.tools ||
                      selectedAgent.tools.length === 0) &&
                      (!selectedAgent.enabled_tools ||
                        selectedAgent.enabled_tools.length === 0) && (
