...
const Protection = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:8000/api/dashboard/realtime-stats', {
        headers: {
    ...
    <div className="min-h-screen bg-[#0a0f1e]">
      ...
      <aside className="fixed left-0 top-0 h-screen w-64 bg-[#0B1120] border-r border-gray-800 p-6">
        ...
        <nav className="space-y-2">
          <a href="/dashboard" className="flex items-center gap-3 px-4 py-3 text-gray-400 hover:text-white hover:bg-gray-800/50 rounded-lg font-medium">
            <span>📊</span>
      ...
      <main className="ml-64 p-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-black text-white mb-2">Protection Hub</h1>
              <p className="text-gray-400">Unified control center for LLM security posture and OWASP mitigation.</p>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
              <span className="text-gray-400 text-sm">
                Live • Updated {lastUpdate ? new Date(lastUpdate).toLocaleTimeString() : 'now'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-6 mb-8">
          <div className="bg-[#0f1629] rounded-xl border border-gray-800 p-6">
            <div className="text-gray-400 text-sm mb-2">Total Threats Blocked</div>
            <div className="text-4xl font-black text-white mb-1">
              {dashboardData ? formatNumber(dashboardData.total_threats_blocked) : '0'}
            </div>
          ...
          <div className="bg-[#0f1629] rounded-xl border border-gray-800 p-6">
            <div className="text-gray-400 text-sm mb-2">Prompt Latency (Avg)</div>
            <div className="text-4xl font-black text-white mb-1">
          ...
          <div className="bg-[#0f1629] rounded-xl border border-gray-800 p-6">
            <div className="text-gray-400 text-sm mb-2">PII Leakage Attempts</div>
            <div className="text-4xl font-black text-white mb-1">
          ...
          <div className="bg-[#0f1629] rounded-xl border border-gray-800 p-6">
            <div className="text-gray-400 text-sm mb-2">Agent Tokens Protected</div>
            <div className="text-4xl font-black text-white mb-1">
              {dashboardData ? formatNumber(dashboardData.agent_tokens_protected) : '0'}
            </div>
            <div className="text-blue-400 text-sm">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-xs">Prompt Injection Filtering</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-xs">Sensitive Data Masking</p>
                </div>
              </div>
              <span className="bg-green-500/20 text-green-400 text-xs font-bold px-3 py-1 rounded-full">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 text-xs">Vector Storage Security</p>
                </div>
              </div>
              <span className="bg-green-500/20 text-green-400 text-xs font-bold px-3 py-1 rounded-full">
          ...
          >
            ...
            <div className="flex items-end gap-2 h-24 mb-4">
              {[50, 70, 55, 80, 65, 90, 75, 85].map((height, i) => (
                <div key={i} className="flex-1 bg-cyan-500 rounded-t group-hover:bg-cyan-400 transition-all" style={{ height: `${height}%` }}></div>
              ))}
            </div>
          ...
          >
            ...
            <div className="space-y-3 mb-4">
              <div className="flex items-center gap-3">
                <div className="flex-1 bg-gray-800 rounded-full h-8 overflow-hidden">
                  <div className="bg-green-500 h-full rounded-full transition-all" style={{ width: '65%' }}></div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex-1 bg-gray-800 rounded-full h-8 overflow-hidden">
                  <div className="bg-green-500 h-full rounded-full transition-all" style={{ width: '45%' }}></div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex-1 bg-gray-800 rounded-full h-8 overflow-hidden">
                  <div className="bg-green-500 h-full rounded-full transition-all" style={{ width: `${dashboardData?.modules?.cost_controls?.budget_utilization || 65}%` }}></div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between text-sm">
              <div>
                ...
                <span className="text-white font-bold ml-2">
                  ${dashboardData?.modules?.cost_controls?.daily_spend?.toFixed(2) || '0.00'}
                </span>
              </div>
              <button 
                onClick={(e) => {
              ...
              <div className="flex items-center justify-center">
                <div className="relative w-24 h-24">
                  <svg className="w-full h-full" viewBox="0 0 100 100">
                    <circle cx="50" cy="50" r="35" fill="none" stroke="#1a1f1e" strokeWidth="8" />
                    <circle 
                      cx="50" cy="50" r="35" 
        ...
        <div className="mt-8 flex items-center justify-between text-sm text-gray-500">
          <div>GATEWAY: {dashboardData?.system?.gateway || '10.0.1.234'} ({dashboardData?.system?.gateway_status || 'Active'})</div>
